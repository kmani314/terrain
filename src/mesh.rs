...
pub fn points_to_mesh(height_map: &[f32], size: u32) -> TriMesh<f32> {
    let mut points = Vec::new();
    let mut indecies = Vec::new();

    for i in 0..size {
        for j in 0..size {
            points.push(Point3::<f32>::new(j as f32, height_map[(i*size + j) as usize], i as f32)); // ArcBall uses y as up :(
